...
if st.session_state.step == "input":
    ...
    with col7: 
        purposes = st.multiselect("✨ 目的", 
            ["秘境探索", "美食・地産地消", "温泉", "歴史・重要文化財", "絶景", "癒やし", "ドライブ", "パワースポット", "穴場巡り"], 
            default=["秘境探索"])
    ...
    if st.button("⚜️ 秘境リサーチを開始する", use_container_width=True, type="primary"):
        ...
        prompt = f"{pref}{city}の{keyword}に合う秘境を10件。名称|解説|住所"
...
# --- STEP 2: スポット選択 ---
    ...
    for i, s in enumerate(st.session_state.found_spots):
        ...
        with col_img:
            st.markdown(f'<div class="spot-placeholder">SPOT IMAGE</div>', unsafe_allow_html=True)
        with col_txt:
            st.markdown(f"**{s[0]}**")
            st.caption(s[1])
    ...
    if st.button("✅ ホテルの調査へ進む", type="primary"): st.session_state.step = "hotel_survey"; st.rerun()
...
# --- STEP 4: プラン生成 ---
    ...
    if not st.session_state.plans:
        with st.spinner("詳細プランを構築中..."):
            for _ in range(5):
                prompt = f"""
                2日間のプランをJSON形式で。
                条件：{st.session_state.selected_spots}採用、ホテル希望は{st.session_state.hotel_wish}
                ルール：各所に「到着時間 - 出発時間」を明記。
                JSON：{{"days": [{{"label": "一日目", "steps": [{{"time": "00:00-00:00", "content": "名称：説明", "is_ai_suggested": false}}]}}], "hotel": {{"name": "名", "address": "住所"}}}}
                """
...
# --- STEP 6: 確定プラン表示＆共有 ---
    ...
    for day in data['days']:
        ...
        for i, step in enumerate(day['steps']):
            ...
            content_rest = step["content"].replace(name, "", 1)
            
            st.info(f"🕒 {step['time']} \n\n {link_html}{content_rest}")
            if i < len(day['steps']) - 1: st.write("　↓")

    st.markdown(f"### 🏨 宿泊先：<a href='https://www.google.com/search?q={urllib.parse.quote(data['hotel']['name'])}' target='_blank' class='confirmed-link'>{data['hotel']['name']}</a>", unsafe_allow_html=True)
    
    st.markdown("""<div class="advice-box"><b>💡 Aipiaの旅のアドバイス</b><br>
    1. 秘境の店は不定休が多いため電話確認を推奨。 2. 特急予約は早割でお得に。 3. モバイルバッテリーを忘れずに。</div>""", unsafe_allow_html=True)

    c1, c2 = st.columns(2)
    with c1: st.markdown(f'<a href="https://line.me/R/msg/text/?確定プラン：{data["hotel"]["name"]}泊" class="reserve-btn" style="background-color:#06C755; width:100%;" target="_blank">LINE共有</a>', unsafe_allow_html=True)
