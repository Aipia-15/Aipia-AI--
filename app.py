...
def call_groq_safe(prompt):
    """
    バグ修正: 応答の確実性を高めるためモデルを調整
    """
    target_models = ["llama-3.3-70b-versatile", "llama3-8b-8192"]
...
st.markdown("""
    ...
    .aipia-logo { font-family: 'Playfair Display', serif; font-size: 3.5rem; color: #111; letter-spacing: 5px; margin: 0; cursor: pointer; }
    ...
    .time-step { background-color: #E3F2FD; padding: 20px; border-radius: 10px; margin: 10px 0; border: 1px solid #BBDEFB; line-height: 1.6; }
...
# --- 3. ヘッダー（タイトル＋サブタイトル） ---
if st.button("Aipia", key="top_title_btn", help="ホームに戻る"):
...
st.markdown('''
    <div class="header-container">
        ...
        <p class="aipia-sub">-AIが創る、秘境への旅行プラン-</p>
    </div>
''', unsafe_allow_html=True)
...
if st.session_state.step == "input":
    ...
    if st.button("⚜️ 秘境リサーチを開始する", use_container_width=True, type="primary"):
        if not pref: st.error("都道府県を選択してください")
        else:
            with st.spinner("秘境を探索中..."):
                st.session_state.form_data = {"dep": dep_place, "dest": f"{pref}{city}", "speed": walk_speed}
                prompt = f"{pref}{city}周辺で{keyword}に関連する秘境を10件。必ず『名称|解説|住所|おすすめ度(1-5)|混雑度(1-5)』の形式で出力せよ。"
                ...
                if content:
                    ...
                    st.error("スポットが見つかりませんでした。キーワードを変えてお試しください。")
...
# --- STEP 2: スポット選択 ---
    ...
    with col_more:
        if st.button("➕ スポットをもっと見る", use_container_width=True):
            with st.spinner("追加リサーチ中..."):
                prompt = f"{st.session_state.form_data['dest']}の別の秘境を10件。名称|解説|住所|4|2"
                res = call_groq_safe(prompt)
                if res:
                    st.session_state.found_spots.extend([l.split('|') for l in res.split('\n') if '|' in l])
                    st.rerun()
    with col_next:
...
# --- STEP 4: プラン生成・編集 ---
    ...
    if not st.session_state.plans:
        with st.spinner("詳細なプランを構築中..."):
            prompt = f"{st.session_state.selected_spots}を含む2日間プランをJSON形式で生成せよ。"
            res = call_groq_safe(prompt)
    ...
    if st.toggle("🛠️ プランを手動編集"):
        st.markdown('<div class="edit-panel">', unsafe_allow_html=True)
        for d_idx, day in enumerate(current_data['days']):
            for s_idx, step in enumerate(day['steps']):
                step['time'] = st.text_input(f"時間 {d_idx}-{s_idx}", step['time'])
                step['content'] = st.text_input(f"内容 {d_idx}-{s_idx}", step['content'])
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    for day in current_data['days']:
        ...
        for i, step in enumerate(day['steps']):
            st.markdown(f'<div class="time-step"><b>{step["time"]}</b><br>{step["content"]}</div>', unsafe_allow_html=True)
            if i < len(day['steps']) - 1: st.markdown('<div class="arrow">↓</div>', unsafe_allow_html=True)

    if st.button("🏆 確定"): st.session_state.step = "share"; st.rerun()
