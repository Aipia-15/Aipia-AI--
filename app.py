...
st.markdown("""
    ...
    .nav-text { font-family: 'Playfair Display', serif; font-size: 2.5rem; color: #111; letter-spacing: 3px; font-weight: 700; margin: 0; }
    ...
    .spot-placeholder { 
        ...
        color: #1B5E20; font-weight: bold; font-size: 1.3rem; text-align: center; padding: 15px;
    }
...
# --- STEP 2: スポット選択 ---
    ...
    st.markdown(f"### 📍 {st.session_state.form_data['dest']} スポット一覧")
    for i, s in enumerate(st.session_state.found_spots):
        ...
        with col_txt:
            ...
            if st.checkbox("このスポットを旅程に入れる", key=f"s_{i}"):
                if name not in st.session_state.selected_spots: st.session_state.selected_spots.append(name)
        st.markdown('</div>', unsafe_allow_html=True)
    
    if st.button("✅ プランのカスタマイズへ", type="primary"): st.session_state.step = "hotel_survey"; st.rerun()

# --- STEP 3 & 4: ホテル・プラン生成 (以下、編集機能など保持) ---
